...
void main()
{
  ...
  vec4 lightColor = max(dot(surf.wNorm, lightDir), 0.0f) * vec4(1.0f, 1.0f, 1.0f, 1.0f);;

  float dstnc = length(Params.lightPos - surf.wPos);
  float attenuation = 1.0f / (dstnc * dstnc);
  float scale = 5.f;
  float s = sampledDepth - posLightSpaceNDC.z;
  float E = max(0.3f + dot(-surf.wNorm, lightDir), 0.0f);
  vec3 transmittance = T(s) * green.rgb * attenuation * E * scale;
